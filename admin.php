...
<head>
    ...
    <style>
        ...
        body { background: var(--dark); color: #fff; font-family: 'Cormorant Garamond', serif; margin: 0; padding: 20px; }
        
        .admin-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(212,175,55,0.3); padding-bottom: 20px; margin-bottom: 30px; }
        .admin-header h1 { font-family: 'Great Vibes', cursive; color: var(--gold); font-size: 3em; margin: 0; }

        /* Stats Grid */
        .stats-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 40px; }
        .stat-box { background: var(--card); border: 1px solid rgba(212,175,55,0.2); padding: 20px; border-radius: 15px; text-align: center; }
        .stat-box h3 { color: var(--gold); text-transform: uppercase; font-size: 0.8em; letter-spacing: 2px; margin-bottom: 10px; }
        .stat-box p { font-size: 2.5em; font-weight: bold; margin: 0; }

        /* Table Styles */
        .order-table { width: 100%; border-collapse: collapse; background: var(--card); border-radius: 15px; overflow: hidden; }
        .order-table th { background: rgba(212,175,55,0.1); color: var(--gold); padding: 15px; text-align: left; text-transform: uppercase; font-size: 0.8em; }
        .order-table td { padding: 15px; border-bottom: 1px solid rgba(255,255,255,0.05); }
        ...
        .btn-action { background: transparent; border: 1px solid var(--gold); color: var(--gold); padding: 5px 10px; border-radius: 5px; cursor: pointer; transition: 0.3s; }
...
<body>
    ...
    async function markAsComplete(id) {
        ...
        const response = await fetch('process_payment.php', {
            ...
            body: JSON.stringify({ order_id: id, method: 'Admin Manual Override' })
        });
