...
<head>
  ...
  <style>
    ...
    .cart-popup { position: fixed; bottom: 28px; right: 28px; background: #111; border: 1px solid rgba(212,175,55,0.4); border-radius: 14px; padding: 12px 16px; display: flex; align-items: center; gap: 12px; box-shadow: 0 8px 30px rgba(0,0,0,0.35); z-index: 99999; max-width: 300px; transform: translateX(120%); opacity: 0; transition: transform .35s ease, opacity .35s ease; font-family: 'Cormorant Garamond', serif; }
...
<div class="lm-overlay" id="lmOverlay">
  <div class="lm-box">
    ...
    <div class="lm-logo"><h2>Casa De Manila</h2><p>Member Portal</p></div>
...
<audio id="toastSound"><source src="./sounds/notify.mp3" type="audio/mpeg"></audio>
...
function requireLogin() {
  if (!document.querySelectorAll('#cartItems .cart-item').length) {
    showToastStack('🛒 Your cart is empty!', 'error'); return;
  }
  toggleCart(); setTimeout(openLM, 250);
}
...
async function submitLogin() {
  ...
  if (res.success) {
    currentUser = res.user; closeLM(); updateNavbar(); updateCheckoutBtn();
